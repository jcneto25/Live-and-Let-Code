...
| **4. Workflows** | Pipeline, gates de validacao e orquestracao | 23 steps pipeline + 2 delta, 26 human gates + checkpoint visual, execution waves, PRRS, dependency matrix |
...
|---------------------------|-------------------|
| **1. Waterfall rígido e lento** — documentação pesada antes de qualquer código, 10x mais lento | O LLC **não é waterfall**. As 23 etapas são pipeline, não fase congelada. PRPs têm 2-8 dias e rodam em **ondas paralelas** assim que validados. O Grill Me é uma rodada curta de perguntas (~15 min), não meses de documentação. O MVP mockado (Step 8) entrega algo funcional e demonstrável em dias, não meses |
...
São pontos de validação humana obrigatórios no pipeline LLC. Nenhum step avança sem aprovação explícita do usuário. O LLC tem 26 human gates + 1 checkpoint visual (subfluxo de prototipagem) + checkpoints de QA na execução. Um gate reprovado retorna o fluxo ao passo anterior com `<gate_result decision="rejected">` registrado no ACE.
...
- **26 human gates:** um apos cada step de geracao (0.5 a 11.3). O humano revisa o artefato e decide: `approved`, `rejected` ou `conditional`
...
| **3. Quality gates em cada transição** | 26 human gates + 1 checkpoint visual + QA gates + Gate 11-ARCH (fitness functions) — nenhum artefato avança sem validação |
...
| Dívida técnica invisível | 26 human gates + QA checkpoints: cada artefato passa por validação explícita antes de prosseguir |
...
| **Aprovar releases** | 26 human gates + QA checkpoints: cada artefato e cada onda passam por validação explícita |
...
O LLC tem **4 skills de seguranca** que operam em momentos diferentes do pipeline, formando uma barreira de protecao em camadas:

| Skill | Step | Quando executa | O que verifica | Gate |
|-------|------|---------------|----------------|------|
| `llc-step-5d-secure-by-design` | Step 5d | **Design** (durante arquitetura) | 10 hard gates, threat modeling, safe code templates, fitness functions | Bloqueia em violacoes de design |
...
**Por que 4 skills separadas?**

1. **Prevencao no design primeiro (Step 5d):** Antes de qualquer decisao de implementacao, o pipeline estabelece 10 hard gates de seguranca (NUNCA hardcode secrets, NUNCA SQL com interpolacao, etc.), threat modeling obrigatorio e safe code templates. Isso garante que o agente carrega o framework de seguranca antes de gerar qualquer linha de codigo.

2. **Ferramentas automatizadas depois (Step 10.6):** Antes de escrever uma linha de codigo, o pipeline verifica se as dependencias tem CVEs, se ha secrets expostos e se o codigo existente tem padroes inseguros. Isso evita que o time construa sobre uma base vulneravel.

3. **Contratos de dados seguros (Step 10.7):** Campos sem especificacao de nulabilidade sao a principal causa de `NullPointerException` e `Cannot read properties of null` em producao. O Step 12 valida que todo campo nos PRPs declara explicitamente se pode ser nulo e, se puder, qual o fallback. Isso previne a classe mais comum de bugs em producao antes de eles serem escritos.

4. **Raciocinio manual/IA depois do codigo (Step 11.1):** Ferramentas automatizadas nao respondem perguntas como "este endpoint verifica se o usuario logado e o dono do recurso?" ou "este reset de senha e de uso unico?". O Step 11.1 inspeciona o codigo implementado com as 10 categorias OWASP, exigindo evidencias arquivo:linha para cada verificacao.
...
23 etapas pipeline + 2 delta (25 no total), 27+ skills (incluindo 1 subfluxo composto de prototipagem + 2 delta + 1 smart-skip). O pipeline vai da ingestao de conhecimento de negocio ao deploy, com suporte a mudancas em sistemas existentes via fluxo delta:
...
> 12-NULL (contratos de dados), 11-OWASP (hardening post-code).
...
### O LLC funciona para stacks que nao sao JavaScript/TypeScript?

Sim. O LLC e **tool-agnostic por design** — skills sao arquivos Markdown, scripts ACE sao Python, gates sao processos humanos. Nenhum componente do pipeline depende de Node.js ou npm.

**Onde o JS/TS aparece (e o equivalente para outras stacks):**

| Componente LLC | Exemplo JS/TS | Equivalente Python | Equivalente Go | Equivalente Rust |
|---------------|--------------|-------------------|----------------|-----------------|
...
| Screenshots (User Guide) | Playwright | Playwright (funciona com Python) | Chromedp | Chromedp |
| Steering files | `CLAUDE.md` / `AGENTS.md` | Idem — Markdown puro | Idem | Idem |
