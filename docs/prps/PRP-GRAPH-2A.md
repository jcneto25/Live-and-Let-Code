...
- [x] `GraphEngine.parallel_frontier()` — retorna lista de nós `auto_parallelizable=True` mutuamente independentes
- [x] Testes garantindo que nós com arestas entre si nunca aparecem juntos na frontier
- [x] Integração com `wave_coordinator.py` (PRP-WAVE-COORD lê este output — consumidor downstream, fora do escopo deste PRP)
...
- [x] Independência mútua garantida por teste (RF-G2A.2 — crítico)
- [x] Sem side-effects, sem chamadas a runtime externo
...
Entregue via TDD (sessão ACE `2026-08-06-018`, step 10.9 — Domain Modeling):

- **`GraphEngine.parallel_frontier()`** — dados puros agnósticos de runtime
  (Q2). Seleção gulosa determinística sobre `ready_nodes()`: filtra
  `auto_parallelizable=True` e `requires_human=False`, e inclui cada nó
  apenas se não tiver aresta com nenhum já selecionado (novo helper
  `_has_edge_between()` cobre DEPENDS_ON/PRODUCES/BLOCKS/REWORK em ambas
  as direções). Nada é executado; nenhum runtime é invocado.
- **Testes:** 7 novos (5 RFs + independência por pares + grafo real do
  `GraphBuilder` com PRPs N2 do dependency-graph.yaml).
- **DIP:** sem imports novos; `llc_wizard` só aparece em docstring.
  `wave_coordinator.py` (PRP-WAVE-COORD) é consumidor downstream — fica
  fora do escopo, como previsto no PRP §1.3.
- **7 testes novos**; suite do pacote 64 verdes. Suite completa **448 passed**,
  fitness **41/41**, cobertura TOTAL **97%** (engine 91%).
