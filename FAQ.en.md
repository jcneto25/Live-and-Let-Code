...
### What are fitness functions and how does LLC implement them?

Fitness functions (Ingeno, Software Architect's Handbook ch16) are **automated checks** that validate architectural characteristics of the code. Unlike unit tests (which validate behavior), fitness functions validate structure: are dependencies correct? Are layers isolated? Are there cycles between modules?

LLC implements **40 fitness functions** in the `.ace/scripts/fitness-functions.py` script, across 5 categories:

| Category | Checks | Flags |
|----------|:---:|-------|
| **Architecture** | 6 | `--check-deps`, `--check-circular`, `--check-interfaces`, `--check-domain`, `--check-usecase`, `--check-coverage` |
| **Clean Code** | 16 | `--check-functions`, `--check-naming`, `--check-classes`, `--check-errors`, `--check-smells`, `--check-readmodels` |
| **Deep Clean** | 8 | `--check-deep-clean` (CQS, `return null`, data clumps, flag arguments, primitive obsession, missing validation, pass-through) |
| **Security** | 5 | `--check-security` (secrets, SQL injection, AsyncStorage tokens, client-only auth, user_id in tables) |
| **UX** | 5 | `--check-ux` (loading states, error feedback, empty states, a11y, responsiveness) |

The 6 original architectural fitness functions:

| Fitness Function | What it checks | Mode |
|-----------------|----------------|------|
| **Dependency Rule** (`--check-deps`) | Services don't import Prisma/infra directly | 🔴 Block (core) / 🟡 Warn |
| **Circular Dependencies** (`--check-circular`) | No NestJS module imports another in a cycle | 🔴 Block (all) |
| **Interface Coverage** (`--check-interfaces`) | Every `.service.ts` has an `I{name}.interface.ts` | 🔴 Block (core) / 🟡 Warn |
| **Domain Isolation** (`--check-domain`) | Files in `domain/` don't import infra | 🔴 Block (core) / 🟡 Warn |
| **Use Case Size** (`--check-usecase`) | Services with >8 public methods | 🟡 Warn |
| **Module Coverage** (`--check-coverage`) | Minimum coverage per module (lcov) | 🟡 Warn |

**Hybrid mode:** Modules defined as `core_modules` in `.ace/arch-config.yaml` follow stricter rules (block). Peripheral modules only generate alerts (warn). The config is generated by Step 5 (Architecture) and editable by the team.

```bash
# Run all fitness functions
python .ace/scripts/fitness-functions.py --all

# Strict mode for CI/CD (exit code 1 on violation)
python .ace/scripts/fitness-functions.py --all --strict --json
```

Beyond fitness functions, the `.ace/scripts/llm-validation.sh` script performs **post-generation self-validation**: 8 checks on AI-generated code (5 blocking: hardcoded secrets, SQL with interpolation, `return null` in services, SQL outside repositories, placeholder tests; 3 warnings: delays in tests, `any` in signatures, `console.*`). The agent runs it before reporting any task as done, and the `ace-llm-validation` hook in `.pre-commit-config.yaml` repeats the barrier at commit.
...
LLC dedicates **Step 11.1** (`docs/skills/llc-step-11-owasp-security.md`) exclusively to OWASP Top 10:2021 hardening. Unlike Step 10.6 — which runs automated tools (SCA, SAST, secrets scanning) **before** implementation — Step 11.1 performs manual/AI checks **after** code is written, inspecting controls that tools can't detect.
...
**Difference from Step 10.6 (pre-implementation):** Step 10.6 finds vulnerabilities in dependencies (SCA), insecure code patterns (SAST), and exposed secrets — all automated. Step 11.1 complements with checks that require reasoning: "does this endpoint verify that the logged-in user owns the resource?" or "does this password reset token expire and is it single-use?" Tools can't answer these questions — OWASP hardening can.
...
No. **Step 10.6** runs static security auditing (SCA + SAST + secret scanning) with `npm audit` (or `pip-audit`), Semgrep, and Gitleaks. These tools run locally, are open-source, and require no external infrastructure.
