...
> **EVALS-F2 concluído** — `CodeEvaluator` (CodeQuality) + `aggregate.py`
> (FirstPassRate/ReworkWaste, 14 testes, cobertura 95%, fitness 41/41).
> **GRAPH-1B concluído** — `GraphEngine` (ready_nodes/impact_of/node_state +
> contrato BLOCKS, 13 testes, cobertura 96%, fitness 41/41).
> **EVALS-F3 concluído** — `DocJudge` (LLM-as-judge + rubrics YAML steps
> 0.5/1/2/3/5 + sampling, 15 testes, cobertura 95%, fitness 41/41).
> **GRAPH-1C concluído** — `projections.py` (`to_kanban` + adapter
> `GraphPipelineDataSource`, 17 testes, cobertura 96%, fitness 41/41).
> **Tracks ativos agora:** EVALS-F4 (paralelo) e GRAPH-2A/2B (próximos no
> caminho crítico).
...
| PRP-WIZARD-1.1 | Kanban UI board (toggle K + SLA visual + WIP) | 2 sem | PRP-WIZARD-1A + PRP-EVALS-F1 | ✅ done (2026-08-06) · 13 testes, cobertura 97%, fitness 41/41 |
| PRP-WIZARD-1.2 | Drag & drop backlog + `--export-flow-metrics` + temas | 1 sem | PRP-WIZARD-1.1 | ✅ done (2026-08-06) · 15 testes, cobertura 97%, fitness 41/41 |
...
| PRP-EVALS-F5 | Dashboard Pareto (custo×qualidade) + ranking | 1 sem | PRP-EVALS-F4 | ✅ done (2026-08-06) · 8 testes, cobertura 96%, fitness 41/41 |
...
| PRP-GRAPH-2B | `critical_path()` + métricas de gargalo | 0,5 sem | PRP-GRAPH-2A | ✅ done (2026-08-06) · 10 testes, cobertura 97%, fitness 41/41 |
...
**Paralelismo ATIVO (2026-08-06 — WIZARD-1A/1B, EVALS-F1/F2/F3 e GRAPH-1A/1B/1C concluídos):**
- **EVALS-F4** (Trilha 2 — baselines + regressão; dep `PRP-EVALS-F3 ✅`)
- **GRAPH-2A/2B** (Trilha 3 — `parallel_frontier()` e `critical_path()`; dep `PRP-GRAPH-1B ✅` — próximos no caminho crítico)
- WIZARD-1C (HITL avançado — Artifact Review/Scope/rerun)
...
> **Progresso real (2026-08-06):** **Trilha 0 (Governança) ✅ e Trilha 1 (Wizard) 5/5 ✅ COMPLETA** — WIZARD-1A ✅ (61 testes), **WIZARD-1B ✅** (61 testes, fitness 41/41), **WIZARD-1C ✅** (8 testes, cobertura 97%, fitness 41/41 — `FailureRecoveryScreen` real com 3 opções, `approved`/`feedback` no artifact_review, scope blocking, rerun sem sair da TUI), **WIZARD-1.1 ✅** (15 testes, cobertura 97%, fitness 41/41 — `KanbanBoardWidget` com 6 colunas, toggle K preservando seleção, SLA `card-stale`, WIP `3/2 🔴`, scores Q:86 T:15500, SKIPPED colapsada) e **WIZARD-1.2 ✅** (15 testes, cobertura 97%, fitness 41/41 — drag & drop de prioridade no BACKLOG com bloqueio educativo para outras colunas, `llc wizard --export-flow-metrics` gerando `flow-metrics-{date}.yaml` com cycle/block time + baseline na 1ª execução, temas dark/light via `wizard.tcss`); **Trilha Wizard concluída — todas as 4 trilhas (Governança, Wizard, Evals, Graph) 100%**. **Trilha 2 (Evals) 5/5 ✅ COMPLETA** — EVALS-F1 ✅ (11 testes), **EVALS-F2 ✅** (14 testes, cobertura 95%), **EVALS-F3 ✅** (15 testes, cobertura 95%, fitness 41/41), **EVALS-F4 ✅** (17 testes, cobertura 96%, fitness 41/41) e **EVALS-F5 ✅** (8 testes, cobertura 96%, fitness 41/41 — dashboard Pareto `llc eval --report` + Eval Summary no code-health). **Trilha 3 (Graph) 5/5 ✅ COMPLETA** — GRAPH-1A (21 testes), **GRAPH-1B ✅** (13 testes), **GRAPH-1C ✅** (22 testes, cobertura 96%, fitness 41/41), **GRAPH-2A ✅** (7 testes, cobertura 97%, fitness 41/41) e **GRAPH-2B ✅** (10 testes, cobertura 97%, fitness 41/41 — `critical_path()` Kahn O(V+E) com pesos SKIPPED/GATE + `to_critical_path`/`to_impact_map`); **caminho crítico zerado e pipeline PRP 100% entregue**.
